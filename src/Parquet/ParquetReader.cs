...
public class ParquetReader : ParquetActor, IAsyncDisposable {
    ...
    /// When true, leaves the stream passed in <paramref name="input"/> open after disposing the reader.
    /// </param>
    /// <param name="cancellationToken"></param>
    ...
    /// <summary>
    /// Opens row group reader. Note that this operation is really cheap as all the metadata is already present. It only
    /// gets expensive when you read actual data, not the metadata itself.
    /// </summary>
    /// <param name="index">
    /// Row group index, starting from 0. See <see cref="RowGroupCount"/> to get number of row groups in this file.
    /// </param>
    /// <returns></returns>
