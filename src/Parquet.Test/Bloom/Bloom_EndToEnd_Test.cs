...
namespace Parquet.Test.Bloom {
    ...
    public sealed class Bloom_EndToEnd_Test : TestBase {
        ...
        private static RowGroup MakeRowGroup(ColumnChunk chunk) =>
            new RowGroup { Columns = new List<ColumnChunk> { chunk }, Ordinal = 0 };
        ...
        public async Task Write_With_Bloom_Then_Read_And_Probe() {
            ...
            var footer = new ThriftFooter(schema, totalRowCount: values.Length, new ParquetOptions());
        ...
        public async Task ByteArray_String_Bloom_EndToEnd() {
            ...
            var footer = new ThriftFooter(schema, totalRowCount: data.Length, new ParquetOptions());
